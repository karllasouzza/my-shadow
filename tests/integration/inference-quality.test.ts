...
import { mockBudgetDevice, mockPremiumDevice } from "@/tests/utils/device-simulator";
...
describe("T025: Inference quality — q8_0 vs f16 KV cache", () => {
  ...
  itOnDevice("perplexity difference between q8_0 and f16 is less than 2%", async () => {
...
describe("T026: Inference consistency with q8_0 KV cache", () => {
  itOnDevice("same prompt produces same output for same seed across 3 runs", async () => {
    const { initLlama } = await import("llama.rn");
    const config = generator.generateRuntimeConfig(mockBudgetDevice(), MODEL_PATH);
    const ctx = await initLlama(config);
...
describe("T027: Language quality spot-check (q8_0 — no artifacts)", () => {
  itOnDevice("100-token output on budget tier is coherent text (no corruption)", async () => {
    const { initLlama } = await import("llama.rn");
    const config = generator.generateRuntimeConfig(mockBudgetDevice(), MODEL_PATH);
    const ctx = await initLlama(config);

    const result = await ctx.completion({
      messages: [{ role: "user", content: "Tell me something interesting about the ocean." }],
      temperature: 0.3,
      n_predict: 100,
    });
