import type { ParsedContent, ParseHtmlOptions, ImageInfo, VideoInfo, LinkInfo } from '../types'

const HTML_ENTITIES: Record<string, string> = {
  '&amp;': '&', '&lt;': '<', '&gt;': '>', '&quot;': '"', '&#39;': "'",
  '&nbsp;': ' ', '&copy;': '\u00A9', '&reg;': '\u00AE', '&trade;': '\u2122',
  '&apos;': "'", '&cent;': '\u00A2', '&pound;': '\u00A3', '&yen;': '\u00A5',
  '&euro;': '\u20AC', '&sect;': '\u00A7', '&deg;': '\u00B0', '&plusmn;': '\u00B1',
  '&middot;': '\u00B7', '&mdash;': '\u2014', '&ndash;': '\u2013',
  '&laquo;': '\u00AB', '&raquo;': '\u00BB', '&lsquo;': '\u2018',
  '&rsquo;': '\u2019', '&ldquo;': '\u201C', '&rdquo;': '\u201D',
  '&bull;': '\u2022', '&hellip;': '\u2026',
...
export function parseHtml(html: string, options?: ParseHtmlOptions): ParsedContent {
  const {
    extractOnlySelector,
...
function extractSourceElements(content: string, poster: string, videos: VideoInfo[]): void {
...
function removeDefaultTags(html: string, extraSelectors?: readonly string[]): string {
...
function extractSection(html: string, selector: string): string {
  const classNames = selector.replace(/^\./, '').split(/[\s,]+/).filter(Boolean)
